...
function ATeamScoreCard({
  ...
  const over = `${Math.floor(bTeamBatterStats.ballsFaced / 6)}.${
    bTeamBatterStats.ballsFaced % 6
  }`;
  console.log(bTeamFallOfWicket&&bTeamFallOfWicket,"igiy")
    ...
    <div className="w-full mb-10  ">
      {secondInningStarted ? (
        <div className="w-full  flex flex-col">
          ...
          <Divider></Divider>
          <TotalRunsCard
            totalRuns={bTeamBatterStats.totalRuns}
            wickets={
              bTeamBatterStats.bTeambattingStats.filter(
          ...
          <Divider></Divider>
          {
            bTeamPlayers.filter((item:any)=>                item.methodOfDismissal == "Yet To Bat").length>1&&
            <YetToBat
            TeamPlayers={bTeamPlayers.filter(
              (item: any) =>
         ...
         <WicketsFallen  fallOfWickets={bTeamFallOfWicket}    ></WicketsFallen>
          ...
          <Divider></Divider>
          <BowlingTeamStats
            bowlingStat={aTeamBowlerStats.aTeamBowlingStats}
          ></BowlingTeamStats>
        </div>
      ) : (
