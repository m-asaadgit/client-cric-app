...
interface AddNewBatterProps {
  firstInninfInSuperOver: boolean;
  batterList: string[];
  verifyIsAdmin: () => Promise<void>;

}
const AddNewBatterInSuperOver = ({
  ...
  const [formData, setFormData] = useState({
    ...
    firstInnning: firstInninfInSuperOver, // hardcoded to always be true
  });
    ...
    if (token) {
      try {
        const response = await axios.put(
          `http://localhost:3000/api/unlisted-match/super-over-next-batter/${matchId}`,
          {
            batterName: formData.batterName,
            firstInning: formData.firstInnning
          },
          {
            headers: {
      ...
      <div className="mb-4 w-[90%] mx-auto">
        ...
        {dropForNewBatter && (
          <div className="w-full  bg-black border-b-white border-b-[1.3px]">
            {batterList?.map((item: any, index: number) => (
                <button
                  onClick={() => {
                    setFormData((prev) => ({
                      ...prev,
...
export default AddNewBatterInSuperOver;
